...
| 14 | Write/read file lifecycle: `"w"`, `.write()`, close, reopen `"r"`, read back | **Demonstrated, fragile edge** | M3L4 C4 final clean run; generated file has 7 sections. M3L4 note debrief later exposed that truncation occurs at `open(..., "w")`, not `.write()`—missed twice before direct explanation. | string accumulator → disk → verified read-back | cold re-probe truncation timing in M3L5+ |
