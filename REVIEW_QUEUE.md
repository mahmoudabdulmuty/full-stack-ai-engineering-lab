...
| File mode + handle lifecycle (Demonstrated practical flow 2026-08-28; ❌ truncation-timing debrief) | M3L4 note debrief: twice predicted `OLD` would survive a crash after `open(..., "w")` but before `.write()`; direct explanation given. Truncation-on-open remains unproven cold. | M3L5+: create a short report with `"w"`, include a crash-before-write scenario, close/reopen with `"r"`, and predict both contents and writer/closed-handle errors. No scaffold. |
