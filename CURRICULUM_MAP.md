...
|---|---|---|---|---|
| 1. Python core + local automation | Python semantics, debugging discipline, files/data, functions/modules, real budget-capped model call | DeepLearning.AI M1–M4; official Python/provider docs on demand | Cold retention + reproducible personal automation with regression tests, validation/failure handling, and a baseline usefulness check | 🔄 IN PROGRESS (Module 3 coverage complete as Guided; Module 4 next) |
| 2. Production backend | HTTP, FastAPI, Pydantic, SQL/PostgreSQL, practical ORM use, pytest, configuration/secrets, errors/logging, Docker, basic CI/CD/deployment/security | Current official documentation, taught through one project | Tested public API with explicit database evidence, a thin Vue/Quasar slice, logs, visible failures, and baseline deployment security | ⬜ |
| 3. AI application patterns | structured outputs, validation, streaming, retries, cost/latency, evaluation; retrieval/tools/agents only when justified | Current provider docs + selected durable *AI Engineering* concepts | Measured AI feature with held-out evaluation, controlled failures, and defensible architecture | ⬜ |
...
| 5. Reliability + differentiators | observability, eval automation, incident response, practical AI security, threat model | Official docs + measured product needs | Pilot feedback, an evidence-driven improvement, recovery practice, production-like reliability/security evidence, and project defense | ⬜ |
